...
from deciphon_api.errors import NotFoundInSchedError, FileNotInStorageError
...
# async def get_scans():
#     with sched(get_sched()) as sched:
#         return sched.exec(select(Scan)).all()
#
#
...
#     sched: sched = Depends(get_session),
# ):
#     return get_hmmer_result(sched.get(Scan, scan_id), prod_id).targets()
#
#
# @router.get(
...
#     sched: sched = Depends(get_session),
# ):
#     return get_hmmer_result(sched.get(Scan, scan_id), prod_id).domains()
#
#
# @router.get(
...
#     sched: sched = Depends(get_session),
# ):
#     return get_hmmer_result(sched.get(Scan, scan_id), prod_id).targets_table()
#
#
# @router.get(
...
#     sched: sched = Depends(get_session),
# ):
#     return get_hmmer_result(sched.get(Scan, scan_id), prod_id).domains_table()
#
#
